...
void Optimizer::getParams()
{
  ...
  if (s.base_constraints.ax_min > 0.0) {
    ...
    RCLCPP_WARN(
      logger_, "Sign of the parameter ax_min is incorrect, consider setting it negative.");
  }
  ...
  models::Trajectories& trajectories, const models::State& state) const
{
  ...
  xt::noalias(trajectories.x) =
    state.pose.pose.position.x + xt::cumsum(dx * settings_.model_dt, {1});
  xt::noalias(trajectories.y) =
    state.pose.pose.position.y + xt::cumsum(dy * settings_.model_dt, {1});
}

xt::xtensor<float, 2> Optimizer::getOptimizedTrajectory()
{
  const bool is_holo = isHolonomic();
  auto&& sequence = xt::xtensor<float, 2>::from_shape({settings_.time_steps, is_holo ? 3u : 2u});
...
void Optimizer::setMotionModel(const std::string& model)
{
  if (model == "DiffDrive") {
    ...
    throw nav2_core::ControllerException(std::string(
      "Model " + model +
